package net.unnamed.service.gui.api.layer.pagination;
...
public class PaginatedInventoryLayer<T extends Item> extends SimpleInventoryLayer {
    @Builder.Default
    int currentPage = 1;
    Function<PageData, Set<T>> pageSupplier;
    Integer itemCount;
    Integer pageCount;
    ...
    public void onDraw(InventoryViewer viewer, ServiceInventory inventory) {
        refreshPaginatedContent(viewer, inventory);
    }

    public void refreshPaginatedContent(InventoryViewer viewer, ServiceInventory inventory) {
        List<T> items = getComparedItems(viewer);
    ...
    public List<T> getComparedItems(InventoryViewer viewer) {
        return pageSupplier.apply(new PageData(viewer, currentPage)).stream()
                .sorted(itemSort)
                .toList();
    }
